...
// Simple Local Authentication System (Fallback)
let localUsers = [];
try {
    localUsers = JSON.parse(localStorage.getItem('eduportal_users') || '[]');
} catch (e) {
    console.error('⚠️ Failed to parse local users from storage:', e);
}

const LocalAuthSystem = {
    users: localUsers,
    
    saveUsers() {
        localStorage.setItem('eduportal_users', JSON.stringify(this.users));
    },
    
    checkAdminLogin(email, password) {
        if (email === ADMIN_CREDENTIALS.email && password === ADMIN_CREDENTIALS.password) {
            const session = { user: ADMIN_CREDENTIALS.user };
    ...
    async signIn(email, password) {
        ...
        const adminCheck = this.checkAdminLogin(email, password);
        if (adminCheck) {
            return adminCheck;
        }
    ...
    async getSession() {
        const sessionStr = localStorage.getItem('eduportal_session');
        if (sessionStr) {
            return { 
...
// Authentication System with Node/Express & MongoDB Atlas
class AuthSystem {
    constructor() {
        ...
        try {
            ...
            if (token) {
                ...
                if (response.ok) {
                    ...
                    // Sync dashboard data
                    if (this.currentUser.dashboardData) {
                        this.syncDashboardFromDatabase(this.currentUser.dashboardData);
                    }
                    
                    this.showMainApp();
                    return;
                } else {
                    console.warn('⚠️ Session token invalid or expired');
                    localStorage.removeItem('eduportal_token');
                }
            }

            // Fallback to local authentication check
            console.log('📱 Checking local authentication system fallback');
            const result = await LocalAuthSystem.getSession();
            if (result.data && result.data.session && result.data.session.user) {
                this.currentUser = result.data.session.user;
                this.isAuthenticated = true;
                console.log('✅ Restored local session for:', this.currentUser.email);
                this.showMainApp();
                return;
            }
    ...
    bindEvents() {
        console.log('🔗 Binding auth events without cloning...');
        ...
        console.log('✅ All auth events bound safely');
    }
    ...
    async handleLogin() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                this.showToast(result.message || 'Invalid email or password.', 'error');
            } else {
                console.log('✅ Login successful:', result.user.email);
                
                // Store JWT token and session
                localStorage.setItem('eduportal_token', result.token);
                ...
                // Sync dashboard data from user record
                if (result.user.dashboardData) {
                    this.syncDashboardFromDatabase(result.user.dashboardData);
                }
                
                this.showToast(email === ADMIN_CREDENTIALS.email ? 'Welcome Admin! 👑' : 'Welcome back!', 'success');
                this.showMainApp();
            }
        } catch (error) {
            console.error('❌ Unexpected login error:', error);
            this.showToast('Login failed. Server may be offline.', 'error');
        }
    ...
    async handleSignup() {
        ...
        try {
            ...
            this.showToast('Signup failed. Server may be offline.', 'error');
        }
    ...
    async showMainApp() {
        ...
        if (this.currentUser) {
            ...
            if (userNameElement) {
                const userName = this.currentUser.name || this.currentUser.user_metadata?.full_name || this.currentUser.email?.split('@')[0];
                userNameElement.textContent = userName;
                console.log('👤 User name set:', userName);
            }
        }
        
        // Initialize main app with a small delay to ensure DOM is ready
        setTimeout(() => {
            ...
            if (typeof window.showHomePage === 'function') {
                ...
                console.log('📍 Showing default home view');
    ...
    async logout() {
        try {
            ...
            // Clear local cached dashboard state
            localStorage.removeItem('savedOpportunities');
            localStorage.removeItem('applicationTracking');
            ...
            if (manager) {
                manager.savedOpportunities = [];
                manager.applicationTracking = [];
                manager.deadlines = [];
                manager.reminders = [];
            ...
            this.showToast('An unexpected error occurred during logout', 'error');
        }
    }

    syncDashboardFromDatabase(dbData) {
        if (!dbData) return;
        console.log('🔄 Syncing dashboard data from database...', dbData);
        const manager = window.dashboardManager;
        if (manager) {
            if (dbData.savedOpportunities) manager.savedOpportunities = dbData.savedOpportunities;
            if (dbData.applicationTracking) manager.applicationTracking = dbData.applicationTracking;
            if (dbData.deadlines) manager.deadlines = dbData.deadlines;
            if (dbData.reminders) manager.reminders = dbData.reminders;
            if (dbData.userPreferences) manager.userPreferences = dbData.userPreferences;
            
            // Write to local storage for local component caches
            localStorage.setItem('savedOpportunities', JSON.stringify(manager.savedOpportunities));
            localStorage.setItem('applicationTracking', JSON.stringify(manager.applicationTracking));
            localStorage.setItem('deadlines', JSON.stringify(manager.deadlines));
            localStorage.setItem('reminders', JSON.stringify(manager.reminders));
            localStorage.setItem('userPreferences', JSON.stringify(manager.userPreferences));
        }
    }
    ...
    async signInWithGoogle() {
        ...
        try {
            ...
            localStorage.setItem('eduportal_session', JSON.stringify({ 
                user: googleUser 
            }));
            ...
            this.showToast('Google sign-in failed. Please try regular login.', 'error');
            this.showLoading(false);
        }
    }
    ...
    showGoogleAccountChooser() {
        return new Promise((resolve) => {
            // Remove any existing google chooser modal first
            const existingModal = document.getElementById('googleChooserModal');
    ...
    showLoading(show) {
        const buttons = document.querySelectorAll('.auth-form .auth-btn');
        buttons.forEach(btn => {
            if (show) {
                btn.disabled = true;
            ...
            <script>
                ...
                document.getElementById('recoveryBtn').addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            showToast(resData.message || 'Password recovery email sent! Check your inbox.');
                            setTimeout(() => window.close(), 3000);
                        } else {
                            showToast(resData.message || 'Failed to send recovery email', 'error');
                        }
                    } catch (err) {
...
function switchAuthForm(formType) {
    ...
    if (window.authSystem) {
        window.authSystem.switchAuthForm(formType);
    } else {
...
function quickAdminLogin() {
    ...
    // Directly use admin credentials
    ...
    if (!emailInput || !passwordInput) {
        ...
        alert('Login form not ready. Please refresh the page.');
        return;
    }
...
class MainApp {
    ...
    init() {
        ...
        if (!this.stateSelect || !this.optionsContainer || !this.loading) {
            console.warn('⚠️ Some MainApp elements not found, retrying...');
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Initializing EduPortal...');
    ...
    // Make authSystem and themeManager globally accessible
    ...
    console.log('✅ Global objects set:', {
        ...
        quickAdminLogin: window.quickAdminLogin,
...
function setupSearch() {
    ...
    function searchAllOpportunities(query) {
        ...
        // Detect primary search category from query
        let priorityCategory = null;
        const queryLower = query.toLowerCase();
        
        if (queryLower.includes('internship') || queryLower.includes('intern') || queryLower.includes('job') || queryLower.includes('placement')) {
            priorityCategory = 'internships';
        } else if (queryLower.includes('scholarship') || queryLower.includes('scholar') || queryLower.includes('grant') || queryLower.includes('financial aid')) {
            priorityCategory = 'scholarships';
        } else if (queryLower.includes('hackathon') || queryLower.includes('hack') || queryLower.includes('competition') || queryLower.includes('contest')) {
            priorityCategory = 'hackathons';
        } else if (queryLower.includes('workshop') || queryLower.includes('training') || queryLower.includes('course') || queryLower.includes('seminar')) {
            priorityCategory = 'workshops';
        }
        
        // Log priority for debugging
        if (priorityCategory) {
            console.log('🎯 Prioritizing category:', priorityCategory, 'for query:', query);
        }
        
        // Sort by category priority first, then by score
        results.sort((a, b) => {
            // If we have a priority category, show those first
            if (priorityCategory) {
                const aMatch = a.category === priorityCategory;
                const bMatch = b.category === priorityCategory;
        ...
        return { error: false, results: results, selectedState: formattedStateName, priorityCategory: priorityCategory };
    }
