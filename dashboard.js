...
class DashboardManager {
    ...
    saveToStorage(key, data) {
        try {
            ...
            // Trigger server sync to MongoDB Atlas
            this.syncWithServer();
        } catch (e) {
    ...
    syncWithServer() {
        ...
        // Debounce server sync to prevent excessive DB writes
        if (this.syncTimeout) {
        ...
        this.syncTimeout = setTimeout(async () => {
            try {
                ...
                if (response.ok) {
                    console.log('🔄 Dashboard state successfully synced to MongoDB Atlas');
                } else {
                    console.warn('⚠️ Dashboard sync returned status:', response.status);
                }
            } catch (err) {
                console.error('❌ Failed to sync dashboard with server:', err);
            }
