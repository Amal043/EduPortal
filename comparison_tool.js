...
class ComparisonTool {
    ...
    removeFromComparison(url) {
        ...
        // If less than 2 items remain, go back to home
    ...
    clearComparison() {
        ...
        if (window.authSystem) {
            window.authSystem.showToast('Comparison cleared', 'info');
        }
    }
    ...
    removeItem(url) {
        this.removeFromComparison(url);
        // Refresh the comparison view if still have items
        if (this.selectedItems.length >= 2) {
        ...
        let html = `
            <div class="comparison-header">
                ...
                <div class="comparison-actions">
                    <button onclick="comparisonTool.clearAll()" class="clear-comparison-btn">
                            ...
                            ${items.map((item, index) => `
                                <th class="item-column">
                                    <div class="item-header">
                                        <h3>${item.name}</h3>
                                        <button onclick="comparisonTool.removeItem('${item.url.replace(/'/g, "\\'")}');" class="remove-item-btn" title="Remove">
